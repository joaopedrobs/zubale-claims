...
export default function ZubalePortal() {
  ...
  useEffect(() => {
    if (state?.success) {
      ["temp_bonus", "temp_data", "temp_turno", "temp_store", "temp_v_rec", "temp_v_anu", "temp_det"].forEach(k => localStorage.removeItem(k));
    }
  }, [state]);
  ...
  // VALIDAÇÃO NO CLIQUE DO BOTÃO (Crucial para funcionar em Prod + UX)
  const handleValidateClick = (e: React.MouseEvent<HTMLButtonElement>) => {
    const errors: Record<string, string> = {};
    const protocoloInput = document.querySelector('input[name="protocolo"]') as HTMLInputElement;
    const protocoloVal = protocoloInput?.value || "";
    const telefoneLimpo = phone.replace(/\D/g, "");

    // 1. Validações
    if (protocoloVal.length < 12) errors.protocolo = "O protocolo deve ter pelo menos 12 dígitos.";
    if (!nome.trim()) errors.nome = "Informe seu nome completo.";
    if (telefoneLimpo.length !== 13) errors.telefone = "DDD e número completos necessários.";
    ...
    // 2. Se houver erro, cancela envio e rola a tela
    if (Object.keys(errors).length > 0) {
      e.preventDefault(); // IMPEDE O SUBMIT NATIVO SE TIVER ERRO
      setFieldErrors(errors);
      const first = Object.keys(errors)[0];
      fieldRefs[first]?.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } else {
      // 3. Se não houver erro, limpa erros e deixa o formulário seguir nativamente
      setFieldErrors({});
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-20 font-sans text-slate-900">
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-6 pt-10 md:pt-16">
        <div className="text-center mb-10 md:mb-12">
          ...
          <p className="text-blue-500 font-bold text-lg md:text-2xl mb-4 italic leading-tight max-w-3xl mx-auto">"Nossa prioridade é garantir que você receba exatamente o que conquistou."</p>
        </div>
        ...
        {state?.success ? <SuccessView /> : (
          <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {/* FORMULÁRIO COM ACTION NATIVA */}
            <form action={(formData) => {
                // Adicionar arquivos manualmente pois eles não vão sozinhos se não for input direto
                selectedFiles.forEach(file => formData.append("evidencias_files", file));
                // O campo LOJA precisa ser setado explicitamente pois é um input controlado
            ...
            >
              <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-white overflow-hidden p-7 md:p-14 space-y-12">
                <section className="space-y-7 md:space-y-8">
                  <SectionHeader number="01" title="SUA IDENTIFICAÇÃO" subtitle="Dados básicos" />
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    <div ref={fieldRefs.protocolo}>
                      ...
                      <p className="text-[11px] font-bold text-slate-400 italic px-2 mt-2 uppercase">* Proibido duplicar protocolos</p>
                    </div>
                    ...
                    <div ref={fieldRefs.telefone}>
                      ...
                      <p className="text-[11px] font-bold text-slate-500 italic px-2 mt-2">* Telefone incorreto impede a identificação.</p>
                    </div>
                ...
                <section className="space-y-7 md:space-y-8 pt-9 border-t border-slate-50">
                  <SectionHeader number="02" title="DADOS DA ATUAÇÃO" subtitle="Sobre o turno" />
                  <div className="space-y-7 md:space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                      <div ref={fieldRefs.data_contestacao}>
                        ...
                        <p className="text-[11px] font-bold text-slate-400 italic px-2 mt-2 uppercase">* Respeite o prazo de 3 dias úteis</p>
                      </div>
                      ...
                      <FieldWrapper label="LOJA ATUADA (LISTA)" error={fieldErrors.loja}>
                        <div className="relative" ref={dropdownRef}>
                          <div className={`relative flex items-center bg-[#f8fafc] border-2 rounded-xl transition-all ${isDropdownOpen ? 'border-blue-500 bg-white ring-4 ring-blue-50' : 'border-[#f1f5f9]'}`} onClick={() => !isLoadingStores && setIsDropdownOpen(true)}>
                            <input type="text" className="w-full p-5 bg-transparent font-bold text-slate-900 outline-none uppercase text-base md:text-lg" value={storeSearch} onChange={(e) => { setStoreSearch(e.target.value); setIsDropdownOpen(true); }} autoComplete="off" />
                ...
                {bonusSelected && (
                  <section className="space-y-7 md:space-y-8 pt-9 border-t border-slate-50 animate-in fade-in">
                    <SectionHeader number="03" title="DETALHES DO REPORTE" subtitle="Valores e evidências" />
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    ...
                    <FieldWrapper label="EVIDÊNCIAS (OPCIONAL)">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {selectedFiles.map((f, i) => (
                          <div key={i} className="flex items-center justify-between p-4 bg-blue-50 border rounded-2xl">
                        ...
                        {selectedFiles.length < 5 && (
                          <label className="flex items-center justify-center gap-3 p-5 border-2 border-dashed rounded-2xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/30">
                            <input type="file" multiple accept="image/*" onChange={handleFileChange} className="hidden" ref={fileInputRef} />
                            <Plus size={22} className="text-slate-400" /> <span className="text-sm font-black uppercase">Anexar</span>
                          </label>
                        )}
                      </div>
                    </FieldWrapper>
              ...
              <div className="p-7 md:p-12 bg-slate-50/50 border-t rounded-[2.5rem]">
                <button type="submit" onClick={handleValidateClick} disabled={isPending || !bonusSelected} className="w-full bg-blue-600 text-white font-black py-5 md:py-7 rounded-[1.5rem] md:rounded-[2rem] hover:bg-blue-700 active:scale-[0.98] transition-all flex justify-center items-center gap-4 text-xl md:text-2xl shadow-xl uppercase">
