...
export default function ZubalePortal() {
  ...
  // Limpa campos temporários após sucesso
  useEffect(() => {
    if (state?.success) {
        ...
        {state?.success ? <SuccessView /> : (
          <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="bg-blue-600 rounded-3xl p-7 md:p-8 text-white shadow-xl shadow-blue-200 border border-blue-500 relative overflow-hidden group">
              ...
              <h3 className="text-xl md:text-xl font-black mb-5 flex items-center gap-2 italic uppercase tracking-tight"><Info size={24} /> Diretrizes Importantes</h3>
            ...
            <form onSubmit={validateAndSubmit} className="space-y-6 md:space-y-8">
              <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-white overflow-hidden p-7 md:p-14 space-y-9 md:space-y-12">
                <section className="space-y-7 md:space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                    <div ref={fieldRefs.protocolo}>
                      ...
                      <p className="text-[11px] font-bold text-slate-400 italic px-2 mt-2 uppercase">* Não reutilize protocolos anteriores</p>
                    </div>
                            ...
                            <div className="absolute z-[60] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden">
                              <div className="max-h-[400px] overflow-y-auto custom-scrollbar">
                                {storesDatabase.filter(s => s.toLowerCase().includes(storeSearch.toLowerCase())).map((loja, i) => (
                                  <div key={i} className="px-6 py-4 hover:bg-blue-50 cursor-pointer text-sm font-bold text-slate-700 border-b last:border-none uppercase transition-colors flex items-center gap-3" onClick={() => { setStoreSearch(loja); localStorage.setItem("temp_store", loja); setIsDropdownOpen(false); setFieldErrors(prev => ({...prev, loja: ""})); }}>
                ...
                {bonusSelected && (
                  <section className="space-y-7 md:space-y-8 pt-9 border-t border-slate-50 animate-in fade-in slide-in-from-top-4 duration-500">
                    ...
                    <FieldWrapper label="EXPLIQUE SEU CASO (OPCIONAL)" icon={<FileText size={20}/>}><textarea name="detalhamento" value={detalhamento} onChange={(e) => {setDetalhamento(e.target.value); localStorage.setItem("temp_det", e.target.value)}} rows={4} className="custom-input resize-none py-5" placeholder="Descreva o ocorrido (Opcional)..." /></FieldWrapper>
